...
// TypeScriptでは複数のクラスの継承、つまり多重継承は許されていない。しかし、複数箇所に分散された機能を集約して使いたい時がある。そんな時に使えるのがinterface。
...
class Jiro implements Kenja, Senshi { // implementは実装という意味。implementsを使うと実装忘れがなくなる。つまり実行中のエラーがなくなる。
  ionazun(): void {
    console.log('ionazun');
  }
