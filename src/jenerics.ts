...
// 上記のような、型が異なるだけでやっていることが同じような関数を共通化したい。そんな時jenericsが使える。

const echo = <T>(arg: T): T => { // <T>がjenericsの宣言。抽象的な型。慣習としてTがよく使われる。型引数と呼ばれる。この時点では何の型かはわからないが、引数の型と戻り値の型は一致している。
  return arg;
}
